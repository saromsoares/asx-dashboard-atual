...
export default function Rastreamento() {
  ...
  return (
    <div className="h-full flex flex-col" style={{ background: 'var(--color-asx-dark)', color: 'var(--color-asx-text)' }}>
      {/* Header */}
      <div className="px-4 md:px-6 py-3 border-b flex items-center gap-3" style={{ borderColor: 'var(--color-asx-border-subtle)' }}>
        <button onClick={() => setLocation('/')} className="flex items-center gap-2 px-3 py-2 rounded-md transition-colors" style={{ background: 'var(--color-asx-surface)', color: 'var(--color-asx-text-heading)' }}>
          <ArrowLeft className="w-4 h-4" /><span className="text-sm font-medium">Menu</span>
        </button>
        <Ship className="w-5 h-5" style={{ color: 'var(--color-asx-red)' }} />
        <span className="font-rajdhani font-bold text-lg tracking-wide" style={{ color: 'var(--color-asx-text-heading)' }}>RASTREAMENTO DE EMBARQUES</span>
      </div>

      <main className="flex-1 overflow-auto p-4 md:p-6">
        {/* KPIs */}
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-8 gap-3 mb-6">
          <div className="p-3 rounded-lg border" style={{ background: 'var(--color-asx-base)', borderColor: 'var(--color-asx-border)' }}>
            <p className="text-[10px] uppercase tracking-wider" style={{ color: 'var(--color-asx-text-muted)' }}>Pedidos Confirm.</p>
            <p className="font-rajdhani font-bold text-2xl" style={{ color: 'var(--color-asx-text-heading)' }}>{kpis.t}</p>
          </div>
          <div className="p-3 rounded-lg border" style={{ background: 'oklch(0.70 0.18 85 / 0.12)', borderColor: 'oklch(0.70 0.18 85 / 0.3)' }}>
            ...
            <p className="font-rajdhani font-bold text-2xl" style={{ color: 'var(--color-asx-warning)' }}>{kpis.pend}</p>
          </div>
          <div className="p-3 rounded-lg border" style={{ background: 'oklch(0.60 0.17 145 / 0.12)', borderColor: 'oklch(0.60 0.17 145 / 0.3)' }}>
            <div className="flex items-center gap-1"><CheckCircle2 className="w-3 h-3" style={{ color: 'var(--color-asx-success)' }} /><p className="text-[10px] uppercase tracking-wider" style={{ color: 'var(--color-asx-success)' }}>Completos</p></div>
            <p className="font-rajdhani font-bold text-2xl" style={{ color: 'var(--color-asx-success)' }}>{kpis.comp}</p>
          </div>
          <div className="p-3 rounded-lg border" style={{ background: 'var(--color-asx-base)', borderColor: 'var(--color-asx-border)' }}>
            <p className="text-[10px] uppercase tracking-wider" style={{ color: 'var(--color-asx-text-muted)' }}>Unid. Pedidas</p>
            <p className="font-rajdhani font-bold text-2xl" style={{ color: 'var(--color-asx-text-heading)' }}>{fmtN(kpis.uPed)}</p>
          </div>
          <div className="p-3 rounded-lg border" style={{ background: 'var(--color-asx-base)', borderColor: 'var(--color-asx-border)' }}>
            <div className="flex items-center gap-1"><Truck className="w-3 h-3" style={{ color: 'oklch(0.60 0.15 200)' }} /><p className="text-[10px] uppercase tracking-wider" style={{ color: 'var(--color-asx-text-muted)' }}>Embarcadas</p></div>
          ...
          <div className="p-3 rounded-lg border" style={{ background: 'var(--color-asx-base)', borderColor: 'var(--color-asx-border)' }}>
            <div className="flex items-center gap-1"><BarChart3 className="w-3 h-3" style={{ color: 'var(--color-asx-red)' }} /><p className="text-[10px] uppercase tracking-wider" style={{ color: 'var(--color-asx-text-muted)' }}>% Embarcado</p></div>
            <p className="font-rajdhani font-bold text-2xl" style={{ color: kpis.pctG >= 90 ? 'var(--color-asx-success)' : kpis.pctG >= 50 ? 'var(--color-asx-warning)' : 'oklch(0.75 0.22 30)' }}>{kpis.pctG.toFixed(1)}%</p>
          </div>
          <div className="p-3 rounded-lg border" style={{ background: 'oklch(0.48 0.22 25 / 0.10)', borderColor: 'oklch(0.48 0.22 25 / 0.3)' }}>
            <p className="text-[10px] uppercase tracking-wider" style={{ color: 'var(--color-asx-error)' }}>USD Pendente</p>
            <p className="font-rajdhani font-bold text-lg" style={{ color: 'var(--color-asx-error)' }}>{fmtUSD(kpis.usd)}</p>
          </div>
        </div>
          ...
          {(['todos', 'pendente', 'completo'] as const).map(f => {
            ...
            return (
              <button key={f} onClick={() => setFiltro(f)} className="px-4 py-2 rounded-md text-sm font-medium transition-colors"
                style={{ background: filtro === f ? 'var(--color-asx-red)' : 'oklch(0.18 0.005 285)', color: filtro === f ? 'white' : 'var(--color-asx-text-secondary)', border: `1px solid ${filtro === f ? 'transparent' : 'var(--color-asx-border)'}` }}>
                {labels[f]}
              </button>
            );
          })}
        ...
        {isLoading ? (
          ...
          <div className="text-center py-12 flex flex-col items-center gap-3" style={{ color: 'var(--color-asx-text-muted)' }}>
          ...
          <div className="space-y-3">
            {pedFilt.map(ped => {
              ...
              return (
                <div key={ped.id} className="rounded-lg border overflow-hidden" style={{ borderColor: bord, background: 'var(--color-asx-base)' }}>
                  {/* Header */}
                  <div className="p-4 flex items-center justify-between cursor-pointer transition-all hover:brightness-110" onClick={() => setPedidoExpandido(exp ? null : ped.id)} style={{ background: 'var(--color-asx-surface)' }}>
                    <div className="flex items-center gap-4 flex-1 min-w-0">
                      <div className="p-2.5 rounded-lg flex-shrink-0" style={{ background: ped.completo ? 'oklch(0.60 0.17 145 / 0.15)' : 'oklch(0.55 0.25 30 / 0.15)' }}>
                        {ped.completo ? <CheckCircle2 className="w-5 h-5" style={{ color: 'var(--color-asx-success)' }} /> : <AlertTriangle className="w-5 h-5" style={{ color: 'oklch(0.75 0.22 30)' }} />}
                      </div>
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-3 flex-wrap">
                          <p className="font-rajdhani font-bold text-base" style={{ color: 'var(--color-asx-text)' }}>{ped.nome}</p>
                          ...
                          {ped.ctnrs.length > 0 && <span className="text-[10px] px-2 py-0.5 rounded" style={{ background: 'var(--color-asx-border-subtle)', color: 'oklch(0.60 0.005 285)' }}>{ped.ctnrs.join(', ')}</span>}
                        </div>
                        <div className="flex items-center gap-4 mt-1 text-xs" style={{ color: 'oklch(0.50 0.010 285)' }}>
                          ...
                          {ped.usdPend > 0 && <><span>•</span><span style={{ color: 'var(--color-asx-error)' }}>{fmtUSD(ped.usdPend)} pend.</span></>}
                        </div>
                      </div>
                    </div>
                    <div className="flex items-center gap-4 flex-shrink-0">
                      <div className="hidden md:flex flex-col items-end gap-1">
                        <div className="w-32 h-2 rounded-full overflow-hidden" style={{ background: 'var(--color-asx-border-subtle)' }}>
                        ...
                        <span className="text-[10px] font-bold" style={{ color: ped.pctG >= 100 ? 'var(--color-asx-success)' : 'oklch(0.65 0.010 285)' }}>{fmtN(ped.tEmbG)}/{fmtN(ped.tPedG)}</span>
                      </div>
                  ...
                  {/* Expanded table */}
                  {exp && (
                    <div className="border-t" style={{ borderColor: 'var(--color-asx-border-subtle)' }}>
                        ...
                        <table className="w-full text-xs">
                            ...
                            {ped.itens.map((it: any, idx: number) => {
                              ...
                              return (
                                <tr key={it.produtoId} style={{ background: idx % 2 === 0 ? 'transparent' : 'oklch(0.14 0.005 285 / 0.5)', opacity: ok ? 0.5 : 1 }}>
                                  <td className="px-3 py-2 font-mono font-bold" style={{ color: 'var(--color-asx-red)' }}>{it.produtoId}</td>
                                  <td className="px-3 py-2 max-w-[220px] truncate" style={{ color: 'var(--color-asx-text-secondary)' }}>{it.descricao}</td>
                                  ...
                                  <td className="px-3 py-2 text-center" style={{ color: it.eS > 0 ? 'oklch(0.70 0.12 200)' : 'var(--color-asx-border-strong)' }}>{fmtN(it.eS)}</td>
                                  <td className="px-3 py-2 text-center" style={{ color: it.eA > 0 ? 'oklch(0.70 0.12 145)' : 'var(--color-asx-border-strong)' }}>{fmtN(it.eA)}</td>
                                  <td className="px-3 py-2 text-center font-bold" style={{ color: it.pendS > 0 ? 'oklch(0.80 0.22 30)' : 'var(--color-asx-border-strong)', background: it.pendS > 0 ? 'oklch(0.55 0.25 30 / 0.08)' : 'transparent' }}>{fmtN(it.pendS)}</td>
                                  <td className="px-3 py-2 text-center font-bold" style={{ color: it.pendA > 0 ? 'oklch(0.80 0.22 30)' : 'var(--color-asx-border-strong)', background: it.pendA > 0 ? 'oklch(0.55 0.25 30 / 0.08)' : 'transparent' }}>{fmtN(it.pendA)}</td>
                                  <td className="px-3 py-2 text-center">
                                    <div className="flex flex-col items-center gap-0.5">
                                      <span className="text-[10px] font-bold" style={{ color: it.pct >= 100 ? 'var(--color-asx-success)' : it.pct > 0 ? 'var(--color-asx-warning)' : 'oklch(0.40 0.010 285)' }}>{it.pct.toFixed(0)}%</span>
                                      <div className="w-10 h-1.5 rounded-full overflow-hidden" style={{ background: 'var(--color-asx-border-subtle)' }}>
                                  ...
                                  <td className="px-3 py-2 text-right font-mono" style={{ color: it.usdPend > 0 ? 'var(--color-asx-error)' : 'var(--color-asx-border-strong)' }}>{fmtUSD(it.usdPend)}</td>
                                  <td className="px-3 py-2 text-center">
                          ...
                          <tfoot>
                            <tr style={{ background: 'oklch(0.11 0.005 285)' }}>
                              ...
                              <td className="px-3 py-2.5 text-center font-bold" style={{ color: ped.pctG >= 100 ? 'var(--color-asx-success)' : 'var(--color-asx-warning)' }}>{ped.pctG.toFixed(0)}%</td>
                              <td className="px-3 py-2.5 text-right font-bold font-mono" style={{ color: 'var(--color-asx-error)' }}>{fmtUSD(ped.usdPend)}</td>
                              <td className="px-3 py-2.5 text-center"><span className="text-[10px] font-bold" style={{ color: ped.completo ? 'var(--color-asx-success)' : 'oklch(0.65 0.010 285)' }}>{ped.iComp}/{ped.totalItens}</span></td>
                            </tr>
                          </tfoot>
                        </table>
                      </div>
                      ...
                      {!ped.completo && (
                        <div className="px-4 py-3 flex items-center justify-between border-t" style={{ borderColor: 'var(--color-asx-border-subtle)', background: 'var(--color-asx-dark)' }}>
